...
def part1():
    ...
    lines = content.split(b'\n')
    pattern_bhp = re.compile(b'(?:[\x00-\xff]*?\x88){4,}')
...
def part4():
    def main():
        ...
        # 保存为Excel
        df.to_excel('PND3output.xlsx', index=False)
        print("Excel文件已生成: PND3output.xlsx")
            ...
            try:
                ...
                record = {
                    ...
                    **full_address,
                    ...
                    'ววันเดือนปีที่จ่าย': thai_date,
                    'ประเภทเงินได้ (200)': match.group('服务类型').strip(),
                    'อัตราภาษี (4 2)': float(match.group('税率')),
    ...
    def process_address(part1: str, part2: str) -> Tuple[dict, str]:
        ...
        # 分割地址
        addr_dict = {
            'บ้านเลขที่': full_address[:30].strip(),
            'ถนน(100)': full_address[30:60].strip() if len(full_address) > 30 else '',
            'แขวง/ตำบล (100)': full_address[60:90].strip() if len(full_address) > 60 else ''
        }
